...
/**
 * 
 * Lambda표현식 java에서 함수형프로그래밍을 지원하기 위한 api(jdk8부터 사용가능)
 * 
 * - 객체지향프로그래밍(상태(field), 행동(method)을 객체로 관리)와 달리 함수를 값으로써 취급한다. - 상태관리를 하지 않고,
 * 모든 것을 불변으로 처리한다.
 * 
 */
...
//		study.test3();
	...
	private void test4() {
		...
		displayNow.run();
		//2. 로또 생성 람다식(1~45 사이의 중복없는 난수 set)
		Supplier<Set<Integer>> lottoPublisher = () -> {
			Set<Integer> lotto_wo = new TreeSet<>();
			while(lotto_wo.size() < 6) {
				lotto_wo.add(new Random().nextInt(45) +1);
			}
			return lotto_wo;
		};
		for(int i = 0; i < 5; i++) {
	...
	/**
	 * JDK가 제공하는 함수형 인터페이스 - 제네릭을 사용해서 람다식 작성 타임에 매개변수나 리턴타입이 결정되도록함.
	 * 
	 ...
	 * 어노테이션을 사용하면, 문법오류를 컴파일타임에 확인 가능
	 */
	public void test2() {
		...
		// 2. 람다식 작성
		// 3. 람다식 호출
		Bar printPerson = (name, age) -> System.out.printf("이름 : %s, 나이 : %d%n", name, age);
...
//		int process(int a);
	...
	/**
	 * 메소드만 전달 또는 변수에 저장이 불가능하므로, 인터페이스를 통해 처리해야한다. lambda식 사용시에는 인터페이스의 추상메소드가 단
	 * 하나만 존재해야한다. 보기에는 메소드만 전달하는 것처럼 보이지만, 실제로는 객체가 전달된 것임.
	 * 
	 */
	private void test1() {
...
//		Pita pita = (int a, int b) -> Math.sqrt(a * a + b * b);
	 ...
	 * 
	 * 자바에서 메소드는 독립적으로 존재할 수 없다. 인자로 전달되거나, 리턴되거나 모두 불가능. 객체를 통해서만 전달 가능. lambda표현식
	 * 또한 이런 제한이 적용.
	 */
	private void test0() {
